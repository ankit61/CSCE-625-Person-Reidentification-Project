...
class SegmentationNetwork(resnet_dilated.Resnet18_8s):
    def __init__(self, weights=None, number_of_classes=2):
        super().__init__(num_classes=number_of_classes)
...
def process_images(dataset_dir, processed_dir, batchsize):
    with SegmentationNetwork('./segmentation/run2/best.pth') as fcn:
        if(batchsize == 1):
